...
| 余额打赏是否允许作者提现 | 是。积分和余额打赏都打开；余额打赏成功后作者收益立即进入可提现余额 | “立即结算”只代表正常订单在事务提交后立即可提现，不代表跳过幂等、账本、风控、审计和违规逆转；作者收益仍必须通过独立子账本与现有混合余额隔离 |
...
| `idea_author_earnings`（余额阶段） | 收益来源、毛额、平台费、净额、冻结时间、可提现时间、结算状态、逆转和提现关联 |
...
- 金额和积分使用 `NUMERIC`，不在账务路径使用 Go `float64` 进行累计。
