...
// Collector stores request-scoped timing samples and is safe for concurrent use.
...
// RecordInterval adds timing without incrementing the operation count.
...
func (c *Collector) record(name string, startedAt, endedAt time.Time, count int) {
	...
	if strings.HasPrefix(name, dependencyPrefix) {
		name = dependencyMetricName(strings.TrimPrefix(name, dependencyPrefix))
	}
...
func dependencyMetricName(module string) string {
	...
	switch module {
	case "github", "openai", "anthropic", "gemini", "grok", "xai", "antigravity", "google", "s3", "payment", "data_managementd":
		return dependencyPrefix + module
	default:
		return dependencyPrefix + "http"
	}
}
...
func normalizeMetricName(name string) string {
	...
	b.Grow(min(len(name), maxMetricNameLen))
	for _, r := range name {
