...
func TestCollectorHeaderValueAggregatesConcurrentIntervals(t *testing.T) {
...
func TestCollectorSanitizesMetricNamesAndNeverLeaksSensitiveInput(t *testing.T) {
	startedAt := time.Unix(400, 0)
	collector := New(startedAt)
	ctx := WithCollector(context.Background(), collector)
	sensitive := "https://user:token@example.test/private?api_key=secret\r\nInjected;dur=999"
	RecordDependency(ctx, sensitive, startedAt, startedAt.Add(time.Millisecond))

	header := HeaderValue(ctx, startedAt.Add(2*time.Millisecond), "bypass")
	for _, forbidden := range []string{"\r", "\n", "https", "user", "token", "private", "api_key", "secret", ";dur=999"} {
		if strings.Contains(header, forbidden) {
			t.Fatalf("unsafe value %q reached header %q", forbidden, header)
		}
	}
	if !strings.Contains(header, "dep_http;dur=") {
		t.Fatalf("unknown dependency name was not collapsed to dep_http: %q", header)
	}
	if len(header) > maxHeaderLen {
		t.Fatalf("header length = %d, want <= %d", len(header), maxHeaderLen)
	}
}

func TestCollectorBoundsHeaderLengthAndMetricIntervals(t *testing.T) {
