...
func TestEffectiveOAuthConfig_EnvSecret(t *testing.T) {
	t.Setenv(GeminiCLIOAuthClientSecretEnv, "test-built-in-secret")

	cfg, err := EffectiveOAuthConfig(OAuthConfig{}, "code_assist")
	if err != nil {
		t.Fatalf("设置环境变量时应使用内置 client_id 和 env secret，实际报错: %v", err)
	}
	if cfg.ClientSecret != "test-built-in-secret" {
		t.Errorf("ClientSecret 应来自环境变量，实际: %q", cfg.ClientSecret)
	}
...
func TestEffectiveOAuthConfig_NoEnvSecret(t *testing.T) {
	...
	if err == nil {
		t.Fatal("不设置环境变量时应返回缺失内置 client_secret 错误")
	}
	if !strings.Contains(err.Error(), GeminiCLIOAuthClientSecretEnv) {
		t.Fatalf("错误信息应包含环境变量名，实际: %v", err)
	}
}
