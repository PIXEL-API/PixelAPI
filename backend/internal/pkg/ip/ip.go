...
// GetClientIP 从 Gin 的可信代理链中提取客户端 IP。
//
// Deprecated: 安全敏感调用应显式使用 GetSecurityClientIP。保留该入口是为了让
// 网关日志和会话种子等既有调用同样遵循可信代理规则，而不是直接信任原始请求头。
...
// GetSecurityClientIP 是登录、注册、风控、ACL 与审计等安全敏感路径的统一入口。
// Gin 仅在请求直连来源命中 server.trusted_proxies 时解析配置的转发头；否则返回
// Request.RemoteAddr 中的直连地址。
