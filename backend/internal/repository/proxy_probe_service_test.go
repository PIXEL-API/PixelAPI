...
func (s *ProxyProbeServiceSuite) TestProbeProxy_Success_IPifyFallback() {
	s.setupProxyServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		// ipify 成功
		if strings.Contains(r.RequestURI, "ipify.org") {
			w.Header().Set("Content-Type", "application/json")
...
func (s *ProxyProbeServiceSuite) TestProbeProxy_AllFailed() {
	...
	// 聚合信息里每个探测点各占一段，且不泄漏完整探测 URL
	msg := err.Error()
	require.Contains(s.T(), msg, "ip-api: ")
	require.Contains(s.T(), msg, "ipify: ")
	require.NotContains(s.T(), msg, "http://")
	require.Less(s.T(), len(msg), 240, "聚合提示应保持精简: %s", msg)
}
...
func TestSummarizeProbeError(t *testing.T) {
	longReason := strings.Repeat("超时原因", 40)
	cases := []struct {
		name   string
		err    error
		expect string
		...
		{"nil", nil, "unknown"},
		{"canceled", fmt.Errorf("proxy connection failed: %w", context.Canceled), "canceled"},
		{"deadline", fmt.Errorf("proxy connection failed: %w", context.DeadlineExceeded), "timeout"},
		{
			"net timeout",
			&url.Error{Op: "Get", URL: "http://api64.ipify.org?format=json", Err: timeoutErr{}},
			"timeout",
		},
		{
			"strips probe url",
			fmt.Errorf("proxy connection failed: %w", &url.Error{
				Op:  "Get",
		...
		{"status code", errors.New("request failed with status: 503"), "request failed with status: 503"},
	}

	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			require.Equal(t, tc.expect, summarizeProbeError(tc.err))
		})
	}

	t.Run("truncates long reason", func(t *testing.T) {
		got := summarizeProbeError(errors.New(longReason))
		require.Equal(t, maxProbeReasonLen+1, len([]rune(got)), "应按 rune 截断并追加省略号")
