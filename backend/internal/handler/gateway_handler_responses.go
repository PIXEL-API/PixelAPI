...
func (h *GatewayHandler) Responses(c *gin.Context) {
	...
	if routeErr != nil {
		...
		h.responsesErrorResponse(c, http.StatusServiceUnavailable, "api_error", "Account routing service is temporarily unavailable")
		return
	}
		...
		for {
			...
			if err != nil {
				...
				if !errors.Is(err, service.ErrNoAvailableAccounts) {
					reqLog.Warn("gateway.responses.account_selection_infrastructure_failed", zap.Error(err))
					h.responsesErrorResponse(c, http.StatusServiceUnavailable, "api_error", "Account selection is temporarily unavailable")
					return
				}
				if len(sessionRejectedAccountIDs) > 0 {
					...
					h.responsesErrorResponse(c, http.StatusServiceUnavailable, "api_error", "No account session capacity is currently available")
					return
				}
			...
			if !selection.Acquired {
				...
				if selection.WaitPlan == nil {
					h.responsesErrorResponse(c, http.StatusServiceUnavailable, "api_error", "Account selection did not provide a concurrency wait plan")
					return
				}
				...
				if err != nil {
					...
					h.responsesErrorResponse(c, http.StatusServiceUnavailable, "api_error", "Account concurrency service is temporarily unavailable")
					return
				}
				if sessionErr := h.gatewayService.RegisterAccountSessionAfterWait(c.Request.Context(), account, sessionHash); sessionErr != nil {
					...
					if errors.Is(sessionErr, service.ErrAccountSessionLimitExceeded) {
						...
						reqLog.Info("gateway.responses.account_session_limit_rejected_after_wait", zap.Int64("account_id", account.ID))
						continue
					}
					reqLog.Warn("gateway.responses.account_session_register_after_wait_failed", zap.Int64("account_id", account.ID), zap.Error(sessionErr))
					h.responsesErrorResponse(c, http.StatusServiceUnavailable, "api_error", "Account session service is temporarily unavailable")
					return
				}
				if err := h.gatewayService.BindStickySession(c.Request.Context(), currentAPIKey.GroupID, sessionHash, account.ID); err != nil {
					reqLog.Warn("gateway.responses.bind_sticky_session_after_wait_failed", zap.Int64("account_id", account.ID), zap.Error(err))
				}
			}
