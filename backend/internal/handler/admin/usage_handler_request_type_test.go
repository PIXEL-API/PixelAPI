...
func TestAdminUsageListUpstreamModelMismatchFilter(t *testing.T) {
	repo := &adminUsageRepoCapture{}
	router := newAdminUsageRequestTypeTestRouter(repo)

	req := httptest.NewRequest(http.MethodGet, "/admin/usage?upstream_model_mismatch=false", nil)
	rec := httptest.NewRecorder()
	router.ServeHTTP(rec, req)

	require.Equal(t, http.StatusOK, rec.Code)
	require.NotNil(t, repo.listFilters.UpstreamModelMismatch)
	require.False(t, *repo.listFilters.UpstreamModelMismatch)
}

func TestAdminUsageListInvalidUpstreamModelMismatch(t *testing.T) {
	repo := &adminUsageRepoCapture{}
	router := newAdminUsageRequestTypeTestRouter(repo)
...
func TestAdminUsageStatsUpstreamModelMismatchFilter(t *testing.T) {
	repo := &adminUsageRepoCapture{}
	router := newAdminUsageRequestTypeTestRouter(repo)

	req := httptest.NewRequest(http.MethodGet, "/admin/usage/stats?upstream_model_mismatch=true", nil)
	rec := httptest.NewRecorder()
	router.ServeHTTP(rec, req)

	require.Equal(t, http.StatusOK, rec.Code)
	require.NotNil(t, repo.statsFilters.UpstreamModelMismatch)
	require.True(t, *repo.statsFilters.UpstreamModelMismatch)
}
