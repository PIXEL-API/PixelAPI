...
// validateCodexImportExtra keeps the upstream Codex import request contract
// without coupling this compatibility facade to service-layer capabilities
// that are not present in this local branch yet.
func validateCodexImportExtra(extra map[string]any) error {
	const longContextBillingKey = "openai_long_context_billing_enabled"
	raw, exists := extra[longContextBillingKey]
	if !exists {
		return nil
	}
	if _, ok := raw.(bool); !ok {
		return infraerrors.BadRequest(
			...
			longContextBillingKey+" must be a boolean",
		)
	}
	return nil
}
...
func (h *AccountHandler) ImportCodexSession(c *gin.Context) {
	...
	if err := validateCodexImportExtra(req.Extra); err != nil {
...
func (h *AccountHandler) importCodexSessions(ctx context.Context, req CodexSessionImportRequest, entries []codexImportEntry) (CodexSessionImportResult, error) {
	...
	index := buildCodexAccountIndex(globalCodexImportAccounts(existingAccounts))
...
// globalCodexImportAccounts keeps the upstream administrator facade inside the
// global-account scope. This local branch also stores user-owned accounts in
// the same account table; allowing them into this identity index could make an
// administrator import update another user's owned credentials.
func globalCodexImportAccounts(accounts []service.Account) []service.Account {
	globalAccounts := make([]service.Account, 0, len(accounts))
	for _, account := range accounts {
		if account.OwnerUserID == nil {
			globalAccounts = append(globalAccounts, account)
		}
	}
