...
// parseTimeRange parses start_time/end_time or start_date/end_date query parameters
// Uses user's timezone if provided, otherwise falls back to server timezone
func parseTimeRange(c *gin.Context) (time.Time, time.Time, error) {
	userTZ := c.Query("timezone") // Get user's timezone from request
	now := timezone.NowInUserLocation(userTZ)
	if startTime, endTime, hasExactTime, err := parseAdminUsageExactTimeRange(c); err != nil || hasExactTime {
		if err != nil {
...
func (h *DashboardHandler) GetStats(c *gin.Context) {
	...
	response.Success(c, gin.H{
		...
		"total_request_actual_cost":   stats.TotalRequestActualCost,
		"total_hourly_cost":           stats.TotalHourlyCost,
		...
		"today_request_actual_cost":   stats.TodayRequestActualCost,
		"today_hourly_cost":           stats.TodayHourlyCost,
