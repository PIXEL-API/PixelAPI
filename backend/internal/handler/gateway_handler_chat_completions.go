	...
	for {
		...
		for {
			...
			if err != nil {
				...
				reqLog.Error("gateway.cc.billing_dispatch_failed", zap.Int64("account_id", account.ID), zap.Error(err))
				h.chatCompletionsErrorResponse(c, http.StatusServiceUnavailable, "api_error", "Billing state is temporarily unavailable; no upstream request was sent")
				return
			}
			userAgent := c.GetHeader("User-Agent")
			clientIP := ip.GetClientIP(c)
