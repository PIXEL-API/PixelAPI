...
func TestHandleFailoverError_CacheBilling(t *testing.T) {
	...
	t.Run("同账号重试时仅凭hasBoundSession不设置ForceCacheBilling", func(t *testing.T) {
		...
		action := fs.HandleFailoverErrorWithRetryLimit(context.Background(), mock, 100, "openai", 1, err)
	...
	t.Run("同账号重试耗尽并实际切换时设置ForceCacheBilling", func(t *testing.T) {
		...
		fs.HandleFailoverErrorWithRetryLimit(context.Background(), mock, 100, "openai", 0, err)
...
func TestHandleFailoverError_ContextCanceled(t *testing.T) {
	t.Run("同账号重试sleep期间context取消", func(t *testing.T) {
		...
		// 进入重试分支后才取消，因此重试计数已递增。
		require.Equal(t, 1, fs.SameAccountRetryCount[100])
	})

	t.Run("入口即已取消_不改动failover状态", func(t *testing.T) {
		mock := &mockTempUnscheduler{}
		fs := NewFailoverState(3, false)
...
func TestHandleFailoverError_IntegrationScenario(t *testing.T) {
	t.Run("模拟完整failover流程_多账号混合重试与切换", func(t *testing.T) {
		...
		require.False(t, fs.ForceCacheBilling, "同账号重试期间不应重复计入缓存费用")
		...
		require.Equal(t, 1, fs.SwitchCount)
		require.Len(t, mock.calls, 1)
		require.True(t, fs.ForceCacheBilling, "同账号重试耗尽并实际换号时应设置 ForceCacheBilling")
...
func TestHandleSelectionExhausted(t *testing.T) {
	...
	t.Run("非503但context已取消_返回Canceled而非Exhausted", func(t *testing.T) {
		fs := NewFailoverState(3, false)
	...
	t.Run("无LastFailoverErr但context已取消_返回Canceled", func(t *testing.T) {
		fs := NewFailoverState(3, false)
...
func TestFailoverClientGone(t *testing.T) {
	...
	t.Run("活跃请求不改状态", func(t *testing.T) {
	...
	t.Run("客户端断开标记499", func(t *testing.T) {
		rec := httptest.NewRecorder()
		c, _ := gin.CreateTestContext(rec)
		ctx, cancel := context.WithCancel(context.Background())
		cancel()
	...
	t.Run("响应已提交不覆盖状态", func(t *testing.T) {
		rec := httptest.NewRecorder()
		c, _ := gin.CreateTestContext(rec)
		ctx, cancel := context.WithCancel(context.Background())
		cancel()
