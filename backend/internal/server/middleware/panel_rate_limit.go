...
//
// 背景：登录后的面板端点此前零限流，单个用户高频刷重聚合查询就能打爆数据库，
// 本项目生产上已因此发生过连接池打满导致的掉线事故。
//
// 设计要点：
//   - 认证接口按「用户 ID」维度计数：与客户端 IP 完全无关，反向代理/共享出口
//     不会互相误伤。
//   - 公开接口按安全客户端 IP 计数：仅统计全局单播地址，回环/内网/链路本地
//     地址直接跳过，避免误拦整条反代链路的流量。
//   - 配置走进程内缓存（60s TTL），热路径零 DB 访问，否则限流本身成为新的压力源。
...
//
// 解析结果为回环/内网/链路本地地址时跳过计数：这类地址通常是反代内部转发地址，
// 按它计数会把整条反代链路的所有真实用户合并进同一个桶，造成大面积误拦截。
