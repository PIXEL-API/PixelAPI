...
func withGrokRequestedModel(ctx context.Context, model string) context.Context {
	...
	return context.WithValue(ctx, grokRequestedModelContextKey{}, model)
}
...
// Keep the name used by the upstream parity tests and team-cooldown code while
// sharing one context key locally.
func withGrokTeamRateLimitModel(ctx context.Context, model string) context.Context {
	return withGrokRequestedModel(ctx, model)
}
...
func persistGrokTransientModelCooldown(account *Account, decision GrokUpstreamFailureDecision) bool {
	if account == nil || decision.Model == "" || !isGrokHeavyTransientModel(decision.Model) {
		return false
	}
	...
	markGrokModelTransientBlock(account.ID, decision.Model, time.Now().Add(cooldown))
	return true
}
...
// BlockModel records whether the decision can be contained to account+model;
// the scheduler filters both account+model and team+model transient blocks.
...
func (s *OpenAIGatewayService) applyGrokUpstreamFailureDecision(
	...
	case GrokFailureBilling:
		...
		if strings.Contains(low, "spending") || strings.Contains(low, "credits") {
			...
			// the account recoverable, surface an explicit reauth hint to admins,
			// and let the normal rate-limit recovery clear schedulability.
			markGrokSpendingLimitReauth(ctx, s.accountRepo, account)
