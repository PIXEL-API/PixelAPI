...
// CNProviderStateRepository is the atomic state boundary for reactive CN
// provider updates. Implementations must compare the loaded account identity
// and scheduling fields before writing, so a stale upstream response cannot
// overwrite an administrator decision or clear another subsystem's block.
type CNProviderStateRepository interface {
	UpdateCNProviderExtraIfMatch(context.Context, int64, *Account, map[string]any) (bool, error)
	SetCNProviderTempUnschedulableIfMatch(context.Context, int64, *Account, time.Time, string) (bool, error)
	ClearCNProviderTempUnschedulableIfMatch(context.Context, int64, *Account, string) (bool, error)
}
...
func (s *RateLimitService) handleCNProviderConcurrencyLimit403(
	...
	repo, ok := s.accountRepo.(CNProviderStateRepository)
	if !ok {
		slog.Warn("cn_concurrency_limit_state_repository_unavailable", "account_id", account.ID)
		return
	}
	if _, err := repo.SetCNProviderTempUnschedulableIfMatch(ctx, account.ID, account, until, reason); err != nil {
...
func cnBalanceLowReason(upstreamMsg string) string {
	// Upstream response bodies are untrusted and may echo credentials. Keep the
	// persisted scheduling reason stable and credential-free.
	_ = upstreamMsg
...
func (s *RateLimitService) handleCNProviderInsufficientBalance(
	...
	repo, ok := s.accountRepo.(CNProviderStateRepository)
	if !ok {
		slog.Warn("cn_balance_low_state_repository_unavailable", "account_id", account.ID)
		return
	}
	if _, err := repo.UpdateCNProviderExtraIfMatch(ctx, account.ID, account, map[string]any{
	...
	if _, err := repo.SetCNProviderTempUnschedulableIfMatch(ctx, account.ID, account, until, msg); err != nil {
...
// cnProviderQuotaSnapshotReset only uses windows whose snapshot confirms
// exhaustion. A reset timestamp alone also exists on healthy quotas and must
// not turn a transient RPM/concurrency error into an hours-long suspension.
// When multiple windows are exhausted, all of them must reset before retry.
func cnProviderQuotaSnapshotReset(account *Account, now time.Time) *time.Time {
	...
	for _, window := range []struct{ used, reset string }{
		...
		used, ok := cnParseF64(account.Extra[cnExtraKey(provider, window.used)])
		if !ok || used < 100 {
			continue
		}
		t := parseCNResetAt(account.Extra[cnExtraKey(provider, window.reset)])
...
func (s *RateLimitService) applyCNProviderReactive429(
	...
	if account.IsQwen() {
		return s.applyQwenReactive429(ctx, account, headers, responseBody)
	}
	// 2) Only confirmed exhausted Coding Plan windows justify a long cooldown.
...
func parseCNResetAt(raw any) *time.Time {
	var value string
	switch v := raw.(type) {
	case string:
