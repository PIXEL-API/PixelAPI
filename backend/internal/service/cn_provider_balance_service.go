...
func (s *CNProviderBalanceService) SetURLSecurityPolicy(settingService *SettingService) {
	if s != nil {
...
func (s *CNProviderBalanceService) queryBalanceForAccount(ctx context.Context, account *Account) (*CNProviderBalanceResult, error) {
	...
	if !gjson.ValidBytes(bodyBytes) {
		result.Error = "Invalid balance response: response is not valid JSON"
		return result, nil
	}
	...
	case PlatformKimi:
		...
		if !code.Exists() || strings.TrimSpace(code.String()) != "0" {
			result.Error = "Invalid balance response: unsuccessful Kimi response"
			return result, nil
		}
		availableBalance := gjson.GetBytes(bodyBytes, "data.available_balance")
		balance, ok := cnParseF64(availableBalance.Value())
		if !availableBalance.Exists() || !ok {
			result.Error = "Invalid balance response: missing or invalid available_balance"
			return result, nil
		}
	...
	if repo, ok := s.accountRepo.(CNProviderStateRepository); !ok {
		slog.Warn("cn_balance_state_repository_unavailable", "account_id", account.ID)
...
func cnBalanceURL(account *Account) string {
	...
	case PlatformKimi:
		if cnURLHostname(account.GetOpenAIBaseURL()) != "api.moonshot.cn" {
			return ""
		}
