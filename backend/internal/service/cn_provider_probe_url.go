...
func cnValidateProbeURL(ctx context.Context, cfg *config.Config, settingService *SettingService, raw string) (string, error) {
	...
	if cfg != nil && cfg.Security.URLAllowlist.Enabled {
		...
		if settingService != nil {
			...
			if err != nil {
				return "", fmt.Errorf("load dynamic upstream allowlist: %w", err)
			}
		}
