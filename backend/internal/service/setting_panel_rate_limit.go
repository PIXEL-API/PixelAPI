...
//
// 登录后的面板接口（/usage、dashboard 聚合等重 SQL 查询）此前没有任何限流，
// 单个用户高频刷就能打爆数据库——本项目生产上已经因此发生过连接池打满导致的掉线事故。
//
// 认证后的接口按「用户 ID」维度限流（与客户端 IP 无关，反向代理/共享出口不会被误伤）；
// 无需认证的公开接口按安全客户端 IP 维度限流（内网/回环地址跳过）。
type PanelRateLimitSettings struct {
	// Enabled 总开关
	Enabled bool `json:"enabled"`
	// UserRPM 每用户每分钟请求数上限（认证面板接口全量计数；0 = 不限制）
	UserRPM int `json:"user_rpm"`
	// HeavyRPM 每用户每分钟重查询上限（usage/dashboard/结算/下单等聚合接口；0 = 不限制）
...
// panelRateLimitRPMMax 面板限流 RPM 的取值上限，防止配置异常大的值失去意义。
...
// cachedPanelRateLimitSettings 进程内缓存条目。
...
// 默认启用但阈值宽松：正常前端交互远达不到，只拦截脚本高频刷接口打爆数据库的行为。
...
func normalizePanelRateLimitSettings(s *PanelRateLimitSettings) {
	...
	clamp := func(v int) int {
		...
		if v > panelRateLimitRPMMax {
			return panelRateLimitRPMMax
		}
		return v
	}
	s.UserRPM = clamp(s.UserRPM)
	s.HeavyRPM = clamp(s.HeavyRPM)
	s.PublicIPRPM = clamp(s.PublicIPRPM)
}

// GetPanelRateLimitSettings 获取面板 API 限流配置（直读 DB，供管理端读写路径使用）。
// 缺失/空/解析失败一律回落默认配置，绝不让配置问题演变成面板不可用。
...
// 否则限流中间件自己就变成新的数据库压力源。
// DB 出错时返回最近一次已知值（无缓存则回落默认值），并以短 TTL 快速重试。
