...
func TestCNValidateProbeURL_AllowlistPolicy(t *testing.T) {
	...
	// 白名单内主机放行（保留完整路径）。
	ok, err := cnValidateProbeURL(context.Background(), cfg, nil, "https://api.moonshot.cn/v1/users/me/balance")
	require.NoError(t, err)
	require.Equal(t, "https://api.moonshot.cn/v1/users/me/balance", ok)

	// 白名单外主机拒绝。
	_, err = cnValidateProbeURL(context.Background(), cfg, nil, "https://relay.attacker.example/v1/usages")
	require.Error(t, err)
	require.Contains(t, err.Error(), "rejected by URL security policy")

	// 私网主机拒绝（内网探测面）。
	_, err = cnValidateProbeURL(context.Background(), cfg, nil, "http://169.254.169.254/latest/meta-data")
	require.Error(t, err)

	// 白名单关闭：仅格式校验，任意 https 主机放行。
	formatOnly, err := cnValidateProbeURL(context.Background(), &config.Config{
		Security: config.SecurityConfig{URLAllowlist: config.URLAllowlistConfig{Enabled: false}},
	}, nil, "https://relay.attacker.example/v1/usages")
	require.NoError(t, err)
	require.Equal(t, "https://relay.attacker.example/v1/usages", formatOnly)
}

func TestCNQuotaOfficialHostMatchingIsExact(t *testing.T) {
	require.Equal(t, "https://evil-bigmodel.cn", zhipuQuotaHost("https://evil-bigmodel.cn"))
	require.Equal(t, "https://open.bigmodel.cn", zhipuQuotaHost("https://open.bigmodel.cn/v4"))
	require.Equal(t, "https://evil-minimax.io/v1/api/openplatform/coding_plan/remains", minimaxQuotaURL("https://evil-minimax.io/v1"))
