...
func (r *cnBalanceProbeRepo) UpdateCNProviderExtraIfMatch(_ context.Context, _ int64, _ *Account, updates map[string]any) (bool, error) {
	r.extraWrites = append(r.extraWrites, updates)
	return true, nil
}

func (r *cnBalanceProbeRepo) SetCNProviderTempUnschedulableIfMatch(context.Context, int64, *Account, time.Time, string) (bool, error) {
	return true, nil
}

func (r *cnBalanceProbeRepo) ClearCNProviderTempUnschedulableIfMatch(context.Context, int64, *Account, string) (bool, error) {
	return true, nil
}
...
func newKimiBalanceProbeAccount() *Account {
	return &Account{
		ID:       43,
...
func TestCNProviderBalanceService_KimiRejectsInvalidPayloads(t *testing.T) {
	tests := []struct {
		name string
		body string
	}{
		{name: "missing code", body: `{"data":{"available_balance":12.5}}`},
		{name: "unsuccessful code", body: `{"code":1,"data":{"available_balance":12.5}}`},
		{name: "missing balance", body: `{"code":0,"data":{}}`},
		{name: "invalid balance", body: `{"code":0,"data":{"available_balance":"unknown"}}`},
		{name: "invalid json", body: `{"code":0`},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			repo := &cnBalanceProbeRepo{account: newKimiBalanceProbeAccount()}
			upstream := &cnBalanceResponseUpstream{statusCode: http.StatusOK, body: tt.body}
			svc := NewCNProviderBalanceService(repo, nil, upstream, nil)
			...
			require.False(t, result.Success)
			require.Empty(t, result.Balances)
			require.Empty(t, repo.extraWrites, "invalid Kimi payload must not persist a synthetic zero balance")
		})
	}
}

func TestCNProviderBalanceService_KimiValidZeroBalanceRemainsSuccessful(t *testing.T) {
	repo := &cnBalanceProbeRepo{account: newKimiBalanceProbeAccount()}
	upstream := &cnBalanceResponseUpstream{
		statusCode: http.StatusOK,
		body:       `{"code":0,"data":{"available_balance":0}}`,
	}
	svc := NewCNProviderBalanceService(repo, nil, upstream, nil)
...
var _ CNProviderStateRepository = (*cnBalanceProbeRepo)(nil)
