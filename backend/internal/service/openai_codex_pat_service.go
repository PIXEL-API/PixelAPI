...
func (s *OpenAIOAuthService) ValidateCodexPersonalAccessToken(ctx context.Context, accessToken, proxyURL string) (*OpenAITokenInfo, error) {
	...
	req.Header.Set("originator", openai.CodexCLIOriginator)
	...
	const maxWhoamiResponseBytes = 64 * 1024
	body, err := io.ReadAll(io.LimitReader(resp.Body, maxWhoamiResponseBytes+1))
	if err != nil {
		return nil, infraerrors.New(http.StatusBadGateway, "OPENAI_CODEX_PAT_RESPONSE_INVALID", "failed to read Codex personal access token validation response")
	}
	if len(body) > maxWhoamiResponseBytes {
		return nil, infraerrors.New(http.StatusBadGateway, "OPENAI_CODEX_PAT_RESPONSE_INVALID", "Codex personal access token validation response is too large")
	}
	decoder := json.NewDecoder(strings.NewReader(string(body)))
	var whoami openAICodexPATWhoamiResponse
	if err := decoder.Decode(&whoami); err != nil {
		return nil, infraerrors.New(http.StatusBadGateway, "OPENAI_CODEX_PAT_RESPONSE_INVALID", "invalid Codex personal access token validation response")
	}
	var trailing any
...
// fields while preserving routing, quota and non-OAuth metadata.
...
func BuildOpenAIPersonalAccessTokenCredentials(tokenInfo *OpenAITokenInfo) map[string]any {
	if tokenInfo == nil {
		return nil
	}
	credentials := map[string]any{
		...
		"chatgpt_account_id":         strings.TrimSpace(tokenInfo.ChatGPTAccountID),
		"chatgpt_user_id":            strings.TrimSpace(tokenInfo.ChatGPTUserID),
		"chatgpt_account_is_fedramp": tokenInfo.ChatGPTAccountFedRAMP,
		"plan_type":                  strings.TrimSpace(tokenInfo.PlanType),
	}
	return NormalizeOpenAIPersonalAccessTokenCredentials(nil, tokenInfo, credentials)
}
...
func isOpenAIPersonalAccessTokenCredentialSet(account *Account, tokenInfo *OpenAITokenInfo, credentials map[string]any) bool {
	...
	return IsOpenAIPersonalAccessTokenCredentials(credentials)
}
