...
func TestShouldFailoverGrokUpstreamError_MethodNotAllowed(t *testing.T) {
	svc := &OpenAIGatewayService{}
	require.True(t, svc.shouldFailoverGrokUpstreamError(
		http.StatusMethodNotAllowed,
		[]byte(`{"error":{"message":"method not allowed"}}`),
	))
}

func TestHandleGrokAccountUpstreamError_MethodNotAllowedHasNoHealthMutation(t *testing.T) {
	repo := &grokQuotaAccountRepo{}
	svc := &OpenAIGatewayService{accountRepo: repo}
	account := &Account{ID: 9106, Platform: PlatformGrok, Type: AccountTypeOAuth}

	svc.handleGrokAccountUpstreamError(
		withGrokRequestedModel(context.Background(), "grok-4.5"),
		account,
		http.StatusMethodNotAllowed,
		nil,
		[]byte(`{"error":{"message":"method not allowed"}}`),
...
func TestHandleGrokAccountUpstreamError_SpendingLimitMarksReauthAndRemainsRecoverable(t *testing.T) {
	repo := &grokQuotaAccountRepo{}
	svc := &OpenAIGatewayService{accountRepo: repo}
	...
	svc.handleGrokAccountUpstreamError(
		...
		nil,
		[]byte(`{"error":{"code":"personal-team-blocked:spending-limit","message":"spending limit reached"}}`),
	)

	require.True(t, accountGrokNeedsReauth(account))
	require.True(t, svc.isOpenAIAccountRuntimeBlocked(account))
	require.Equal(t, true, repo.updates[account.ID]["grok_needs_reauth"])
	require.Equal(t, "spending_limit", repo.updates[account.ID]["grok_needs_reauth_reason"])
	require.WithinDuration(t, time.Now().Add(grokSpendingLimitProbeCooldown), repo.lastRateLimitResetAt, time.Second)
	require.Zero(t, repo.credentialErrorCalls, "spending limit must not become a permanent account error")
}
...
func TestHandleGrokAccountUpstreamError_Entitlement403Unchanged(t *testing.T) {
	...
	// Pixel keeps OAuth credential mutations behind credential-version CAS. The
	// test repository accepts that CAS, so the immediate runtime bridge must be
	// active even though the generic SetTempUnschedulable method is not used.
	require.Zero(t, repo.tempUnschedCalls)
	require.True(t, svc.isOpenAIAccountRuntimeBlocked(account))
}
