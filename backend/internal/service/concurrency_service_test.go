...
func TestAcquireOpenAIWSIngressLease(t *testing.T) {
	...
	t.Run("cache error is returned", func(t *testing.T) {
		cache := &ingressLeaseCacheForTest{acquireIngressErr: errors.New("redis unavailable")}
		lease, acquired, err := NewConcurrencyService(cache).AcquireOpenAIWSIngressLease(context.Background(), 1, 1)
		require.Error(t, err)
		require.False(t, acquired)
		require.Nil(t, lease)
...
func TestOpenAIWSIngressLeaseRefreshLoss(t *testing.T) {
	t.Run("owned lease refreshes confirmation time", func(t *testing.T) {
		cache := &ingressLeaseCacheForTest{refreshIngressResult: true}
		lease := &OpenAIWSIngressLease{cache: cache, apiKeyID: 1, leaseID: "owned"}
		previous := time.Now().Add(-time.Minute)
		confirmedAt, lost := lease.refresh(previous)
		require.False(t, lost)
		require.True(t, confirmedAt.After(previous))
	...
	t.Run("transient redis error is tolerated before ttl", func(t *testing.T) {
		cache := &ingressLeaseCacheForTest{refreshIngressErr: errors.New("redis unavailable")}
		lease := &OpenAIWSIngressLease{cache: cache, apiKeyID: 1, leaseID: "transient"}
		lastConfirmedAt := time.Now().Add(-openAIWSIngressLeaseTTL / 2)
		confirmedAt, lost := lease.refresh(lastConfirmedAt)
		require.False(t, lost)
		require.Equal(t, lastConfirmedAt, confirmedAt)
		require.Equal(t, 1, cache.refreshIngressCalls)
	})
