...
func TestCalculateOpenAI429ResetTime_NeitherExhausted_ReturnsNil(t *testing.T) {
	...
	// Neither limit at 100% proves quota exhaustion; reset-after is only window metadata.
	headers := http.Header{}
	...
	if resetAt != nil {
		t.Errorf("expected nil resetAt when neither window is exhausted, got %v", resetAt)
	}
}
