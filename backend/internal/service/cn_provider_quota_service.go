...
type CNProviderQuotaProbeResult struct {
	...
	CredentialValid         *bool                        `json:"credential_valid"` // nil = not checked; false = authentication failed
	AutomaticQuerySupported *bool                        `json:"automatic_query_supported,omitempty"`
	ObservedLimits          []QwenCodingLimitObservation `json:"observed_limits,omitempty"`
...
func (s *CNProviderQuotaService) SetURLSecurityPolicy(settingService *SettingService) {
	if s != nil {
...
func (s *CNProviderQuotaService) QueryUsageForAccount(ctx context.Context, account *Account) (*CNProviderQuotaProbeResult, error) {
	...
	if account.IsQwen() {
		return qwenCodingQuotaResult(account), nil
	}
...
func (s *CNProviderQuotaService) queryUsageForAccount(ctx context.Context, account *Account) (*CNProviderQuotaProbeResult, error) {
	...
	if targetURL == "" {
		return nil, infraerrors.New(http.StatusBadRequest, "CN_QUOTA_UNSUPPORTED_HOST", "coding plan quota endpoint is unavailable for this base URL")
	}

	// 探测发起前过出站 URL 安全策略（与网关转发/Grok 探测同一套校验）：
	// 端点多由账号 base_url 衍生，不得把 API key 发往策略外主机。
	validatedURL, err := cnValidateProbeURL(ctx, s.cfg, s.settingService, targetURL)
	if err != nil {
	...
	bodyBytes, err := io.ReadAll(io.LimitReader(resp.Body, cnQuotaMaxBodyBytes+1))
	if err != nil {
		return nil, infraerrors.New(http.StatusBadGateway, "CN_QUOTA_RESPONSE_READ_FAILED", "failed to read coding plan quota response")
	}
	if len(bodyBytes) > cnQuotaMaxBodyBytes {
		return nil, infraerrors.New(http.StatusBadGateway, "CN_QUOTA_RESPONSE_TOO_LARGE", "coding plan quota response is too large")
	}
	...
	if !gjson.ValidBytes(bodyBytes) {
		result.Error = "Coding plan quota response is not valid JSON"
		return result, nil
	}
	...
	if len(tiers) == 0 {
		result.Error = "Coding plan quota response contains no usable quota windows"
		return result, nil
	}
	result.Tiers = tiers
	result.Success = true
	...
	if repo, ok := s.accountRepo.(CNProviderStateRepository); !ok {
		slog.Warn("cn_quota_state_repository_unavailable", "account_id", account.ID)
	} else if updated, err := repo.UpdateCNProviderExtraIfMatch(ctx, account.ID, account, updates); err != nil {
...
func minimaxQuotaURL(baseURL string) string {
	...
	default:
		...
		return strings.TrimRight(u.Scheme+"://"+u.Host, "/") + "/v1/api/openplatform/coding_plan/remains"
	}
}

func zhipuQuotaHost(baseURL string) string {
	host := cnURLHostname(baseURL)
	switch host {
...
func cnURLHostname(raw string) string {
...
func parseKimiUsageTiers(body []byte) []CNQuotaTier {
	...
	if limits := gjson.GetBytes(body, "limits"); limits.IsArray() {
		limits.ForEach(func(_, item gjson.Result) bool {
			...
			limit, validLimit := cnParseF64(detail.Get("limit").Value())
			remaining, validRemaining := cnParseF64(detail.Get("remaining").Value())
			if !validLimit || !validRemaining || limit <= 0 || remaining < 0 {
				return true
			}
	...
	if usage := gjson.GetBytes(body, "usage"); usage.Exists() {
		limit, validLimit := cnParseF64(usage.Get("limit").Value())
		remaining, validRemaining := cnParseF64(usage.Get("remaining").Value())
		if !validLimit || !validRemaining || limit <= 0 || remaining < 0 {
			return tiers
		}
		used := limit - remaining
...
func parseZhipuTokenTiers(data gjson.Result) []CNQuotaTier {
	...
	data.Get("limits").ForEach(func(_, item gjson.Result) bool {
		...
		percentage, validPercentage := cnParseF64(item.Get("percentage").Value())
