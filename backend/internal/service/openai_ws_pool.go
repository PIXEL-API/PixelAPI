...
type openAIWSAcquireRequest struct {
	...
	WSURL   string
	Headers http.Header
	// HeadersFactory 在实际拨号前生成一次性认证头，避免把短期凭据缓存进
	// lastAcquire 或延迟预热状态。
...
type openAIWSAccountPool struct {
	...
	// effectiveMaxConns stores only the last computed numeric cap. Keeping the
	// full acquire request here would retain account credentials and ephemeral
	// Agent Identity header factories for the lifetime of the process.
...
func (p *openAIWSConnPool) runBackgroundCleanupSweep(now time.Time) {
	...
	p.accounts.Range(func(_ any, value any) bool {
		...
		ap.mu.Lock()
		if ap.effectiveMaxConns > 0 {
			maxConns = ap.effectiveMaxConns
		}
...
retryAcquire:
	...
	ap.mu.Lock()
	ap.effectiveMaxConns = effectiveMaxConns
	now := time.Now()
	...
	if allowReuse {
		if forcePreferredConn {
			...
			if preferredConn.tryAcquire() {
				...
				p.ensureTargetIdleAsync(accountID, req)
				return lease, nil
			}
...
func (p *openAIWSConnPool) ensureTargetIdleAsync(accountID int64, requests ...openAIWSAcquireRequest) {
	if p == nil || accountID <= 0 {
		return
	}
	...
	ap.mu.Lock()
	defer ap.mu.Unlock()
	var request *openAIWSAcquireRequest
	if len(requests) > 0 && requests[0].Account != nil {
		copied := cloneOpenAIWSAcquireRequest(requests[0])
		request = &copied
	} else if ap.lastAcquire != nil {
		copied := cloneOpenAIWSAcquireRequest(*ap.lastAcquire)
		request = &copied
	}
	...
	effectiveMaxConns := p.effectiveMaxConnsByAccount(request.Account)
	ap.effectiveMaxConns = effectiveMaxConns
...
// ClearAccount 关闭指定账号的所有池连接并废弃延迟预热状态。generation
// 可阻止凭据更新前启动的预热连接在稍后重新进入连接池。
...
func (p *openAIWSConnPool) dialConn(ctx context.Context, req openAIWSAcquireRequest) (*openAIWSConn, error) {
	...
	if req.HeadersFactory != nil {
		...
		if err != nil {
			return nil, fmt.Errorf("build openai ws dial headers: %w", err)
		}
	}
	...
	pooledConn.betaFeatures = normalizeOpenAIWSBetaFeatures(headers)
