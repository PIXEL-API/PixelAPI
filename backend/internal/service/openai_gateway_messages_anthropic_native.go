...
func (s *OpenAIGatewayService) forwardAnthropicViaNativeAnthropicEndpoint(
	...
	if resp.StatusCode >= 400 {
		...
		if s.shouldFailoverOpenAIUpstreamResponse(account, resp.StatusCode, upstreamMsg, respBody) {
			s.handleOpenAIAccountUpstreamErrorForModel(ctx, account, upstreamModel, resp.StatusCode, resp.Header, respBody)
			return nil, newOpenAIUpstreamFailoverError(resp.StatusCode, resp.Header, respBody, upstreamMsg, shouldRetryOpenAIOnSamePoolAccount(account, resp.StatusCode, upstreamMsg, respBody))
		}
...
func (s *OpenAIGatewayService) nativeAnthropicTargetURL(account *Account) (string, error) {
	...
	return buildOpenAIMessagesURL(validatedURL), nil
}
...
func (s *OpenAIGatewayService) handleNativeAnthropicBufferedResponse(
	...
	if err := json.Unmarshal(body, &raw); err != nil {
		return nil, newOpenAIUpstreamFailoverError(resp.StatusCode, resp.Header, body, err.Error(), false)
	}
