...
func (s *CNProviderBalanceCheckService) runLeasedOnce() {
	...
	run := func(taskCtx context.Context, guard *ClusterLeaseGuard) error {
		...
		s.runOnceWithGuard(taskCtx, guard)
		return nil
	}
	if s.taskExecutor == nil {
		runOnceWithGuard := run
		_ = runOnceWithGuard(ctx, &ClusterLeaseGuard{})
		return
	}
	if _, err := s.taskExecutor.Run(ctx, "cn_provider_balance_check", run); err != nil {
		if !errors.Is(err, context.Canceled) {
			log.Printf("[CNBalance] task failed: %v", err)
		}
	}
}
...
func (s *CNProviderBalanceCheckService) runOnceWithGuard(ctx context.Context, guard *ClusterLeaseGuard) {
	...
	collect := func(platform string, accounts []Account) {
		for i := range accounts {
			...
			// 白跑并产生告警噪声。
			if isCNOllamaCloudUsageAccount(account) {
				continue
			}
	...
	for _, account := range paygTargets {
		if err := guard.Check(probeCtx); err != nil {
			return
		}
		switch s.checkOne(probeCtx, account, threshold) {
	...
	if len(quotaTargets) > 0 && s.quotaService != nil {
		...
		for _, target := range quotaTargets {
			...
			go func(t quotaTarget) {
				...
				defer func() { <-sem }()
				if err := guard.Check(probeCtx); err == nil {
					s.probeQuota(probeCtx, t.id, t.platform)
				}
			}(target)
		}
		wg.Wait()
...
// runOnce is retained for deterministic unit tests and local callers. The
// production ticker uses runLeasedOnce so cluster lease fencing is enforced.
func (s *CNProviderBalanceCheckService) runOnce() {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Minute)
	defer cancel()
	s.runOnceWithGuard(ctx, &ClusterLeaseGuard{})
}

func isCNOllamaCloudUsageAccount(account *Account) bool {
	if account == nil {
		return false
	}
	base := strings.ToLower(strings.TrimSpace(account.GetCredential("base_url")))
	return strings.Contains(base, "://api.ollama.com") || strings.Contains(base, "://ollama.com")
}
...
func (s *CNProviderBalanceCheckService) checkOne(ctx context.Context, account *Account, threshold float64) cnBalanceCheckOutcome {
	...
	if low {
		...
		if _, err := repo.SetCNProviderTempUnschedulableIfMatch(ctx, account.ID, account, time.Now().Add(s.cooldown()), reason); err != nil {
	...
	if account.TempUnschedulableUntil != nil && strings.HasPrefix(account.TempUnschedulableReason, cnBalanceLowReasonPrefix) {
		...
		if _, err := repo.ClearCNProviderTempUnschedulableIfMatch(ctx, account.ID, account, cnBalanceLowReasonPrefix); err != nil {
