	...
	if resp.StatusCode >= http.StatusBadRequest {
		...
		if s.shouldFailoverOpenAIUpstreamResponse(resp.StatusCode, upstreamMsg, respBody) {
			...
			if s.rateLimitService != nil {
				s.rateLimitService.HandleUpstreamErrorForModel(ctx, account, upstreamModel, resp.StatusCode, resp.Header, respBody)
			}
			return nil, &UpstreamFailoverError{StatusCode: resp.StatusCode, ResponseBody: respBody, RetryableOnSameAccount: shouldRetryOpenAIOnSamePoolAccount(account, resp.StatusCode, upstreamMsg, respBody)}
		}
