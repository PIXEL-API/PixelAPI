...
describe('user KeysView accessibility interactions', () => {
  ...
  it('focuses and clamps the teleported group selector and restores its trigger focus', async () => {
    ...
    const options = document.querySelectorAll<HTMLButtonElement>(`#${dialogId} .data-teleport-option`)
    expect(options.length).toBeGreaterThan(0)
    expect(option!.classList.contains('p-0.5')).toBe(true)
    expect(options[options.length - 1].classList.contains('last:!border')).toBe(true)
    option!.focus()
    option!.click()
    await nextTick()
    expect(document.getElementById(dialogId)).toBeNull()
