...
import type { IdeaMineStatus } from '@/api/ideas'
...
const error = ref('')
type IdeaStatusFilter = 'all' | IdeaMineStatus

const statusFilter = ref<IdeaStatusFilter>('all')
...
const filters = computed<Array<{ key: IdeaStatusFilter; label: string }>>(() => [
  { key: 'all', label: t('ideas.mine.all') },
  { key: 'draft', label: t('ideas.status.draft') },
  { key: 'pending_review', label: t('ideas.status.pending_review') },
  { key: 'published', label: t('ideas.status.published') },
  { key: 'rejected', label: t('ideas.status.rejected') },
])
...
function onStatusFilter(key: IdeaStatusFilter) {
...
function statusOf(p: IdeaPost) {
  return {
    text: t(`ideas.status.${p.status}`, p.status),
    cls: statusClasses[p.status] || 'bg-surface-subtle text-content-muted border-line',
  }
}
...
async function publish(p: IdeaPost) {
  ...
  try {
    ...
    appStore.showSuccess(t('ideas.mine.submitSuccess'))
    await load()
  } catch (e: any) {
    appStore.showError(e?.message || t('ideas.mine.submitFailed'))
  } finally {
    actingId.value = null
  }
}
...
async function remove(p: IdeaPost) {
  if (!window.confirm(t('ideas.mine.deleteConfirm', { title: p.revision?.title || t('ideas.mine.thisPost') }))) return
  actingId.value = p.id
  try {
    await deleteIdea(p.id)
    appStore.showSuccess(t('ideas.mine.deleteSuccess'))
    await load()
    ...
    appStore.showError(e?.message || t('ideas.mine.deleteFailed'))
  } finally {
    actingId.value = null
  }
}
...
function formatDate(s?: string) {
  if (!s) return ''
  return new Date(s).toLocaleDateString(locale.value, { month: 'short', day: 'numeric' })
}
  ...
  <div class="min-h-screen bg-canvas text-content">
    ...
    <div class="mx-auto max-w-4xl px-4 pb-16 pt-6 md:px-6">
      ...
      <div class="mb-6 flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 class="text-2xl font-bold text-content">{{ t('ideas.mine.title') }}</h1>
          <p class="mt-1 text-sm text-content-muted">{{ t('ideas.mine.description') }}</p>
        </div>
        <RouterLink
          to="/ideas/new"
      ...
      <div v-else-if="error" class="rounded-panel border border-line bg-surface p-10">
        <EmptyState :title="error" :description="t('ideas.common.loadUnavailable')">
          ...
          <template #action><button class="inline-flex h-10 items-center rounded-control bg-brand px-5 text-sm font-semibold text-content-inverse hover:bg-brand-strong" @click="load">{{ t('ideas.common.retry') }}</button></template>
        </EmptyState>
      </div>

      <!-- Empty -->
      <div v-else-if="posts.length === 0" class="rounded-panel border border-line bg-surface p-10">
        <EmptyState :title="statusFilter === 'all' ? t('ideas.mine.emptyAllTitle') : t('ideas.mine.emptyFilteredTitle')" :description="statusFilter === 'all' ? t('ideas.mine.emptyAllDescription') : t('ideas.mine.emptyFilteredDescription')">
          ...
          <template #action>
            <RouterLink to="/ideas/new" class="inline-flex h-10 items-center gap-2 rounded-control bg-brand px-5 text-sm font-semibold text-content-inverse hover:bg-brand-strong">
              <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 5v14M5 12h14" /></svg>{{ t('ideas.common.writePost') }}
            </RouterLink>
          </template>
        </EmptyState>
      </div>
        ...
        >
          <div class="min-w-0 flex-1">
            <div class="flex flex-wrap items-center gap-2">
              <RouterLink :to="`/ideas/${p.id}`" class="line-clamp-1 text-base font-semibold text-content transition-colors hover:text-brand">{{ p.revision?.title || t('ideas.common.untitled') }}</RouterLink>
            ...
            <div class="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-content-subtle">
              <span class="inline-flex items-center gap-1"><svg class="h-3.5 w-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round"><path d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178zM15 12a3 3 0 11-6 0 3 3 0 016 0z" /></svg>{{ formatCount(p.view_count) }} {{ t('ideas.common.views') }}</span>
              <span class="inline-flex items-center gap-1"><svg class="h-3.5 w-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round"><path d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12z" /></svg>{{ formatCount(p.like_count) }} {{ t('ideas.common.likes') }}</span>
              <span class="inline-flex items-center gap-1"><svg class="h-3.5 w-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round"><path d="M17.593 3.322c1.1.128 1.907 1.077 1.907 2.185V21L12 17.25 4.5 21V5.507c0-1.108.806-2.057 1.907-2.185a48.507 48.507 0 0111.186 0z" /></svg>{{ formatCount(p.favorite_count) }} {{ t('ideas.common.favorites') }}</span>
