...
describe('admin account duplicate API', () => {
  ...
  it('sends a stable idempotency key', async () => {
  ...
  it('reuses the operation key after a module reload', async () => {
    post.mockRejectedValueOnce(new Error('network timeout'))
  ...
  it('fails before sending when secure UUID generation is unavailable', async () => {
    vi.spyOn(globalThis.crypto, 'randomUUID').mockImplementation(() => {
      throw new Error('secure random unavailable')
    })

    await expect(duplicate(123)).rejects.toThrow('secure random unavailable')
