...
function createDefaultAuthSourceDefaultsValue(): AuthSourceDefaultsValue {
  return {
...
export function appendAuthSourceDefaultsToUpdateRequest(
  ...
  for (const source of AUTH_SOURCE_TYPES) {
    const current =
      authSourceDefaults[source] ?? createDefaultAuthSourceDefaultsValue();
...
export interface SystemSettings {
  ...
  upstream_url_allowlist_extra_hosts: string[];
...
/**
 * 429 fallback cooldown settings interface
 */
