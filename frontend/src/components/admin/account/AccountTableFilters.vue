<template>
  <div class="flex flex-wrap items-center gap-3">
    ...
    />
    <SearchInput
      :model-value="filters.owner_search || ''"
      :placeholder="t('admin.accounts.searchOwner')"
      class="w-full sm:w-64"
      @update:model-value="updateOwnerSearch"
      @search="$emit('change')"
    />
...
const pOpts = computed(() => [{ value: '', label: t('admin.accounts.allPlatforms') }, { value: 'anthropic', label: 'Anthropic' }, { value: 'openai', label: 'OpenAI' }, { value: 'gemini', label: 'Gemini' }, { value: 'antigravity', label: 'Antigravity' }, { value: 'grok', label: 'Grok' }, { value: 'opencode', label: 'OpenCode' }])
