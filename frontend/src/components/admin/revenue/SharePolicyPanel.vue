<template>
  <div class="space-y-6">
    ...
    <section class="card p-5">
      <div class="mb-4">
        <h3 class="text-base font-semibold text-gray-900 dark:text-white">
          {{ t('admin.revenue.sharePolicy.selfUseCommissionTitle') }}
        </h3>
        <p class="mt-1 text-sm leading-6 text-gray-500 dark:text-gray-400">
          {{ t('admin.revenue.sharePolicy.selfUseCommissionDescription') }}
        </p>
      </div>

      <form class="grid grid-cols-1 gap-4 md:grid-cols-[minmax(0,280px)_1fr] md:items-end" @submit.prevent="saveSelfUseCommissionSettings">
        <div>
          <label class="mb-1 block text-xs font-medium text-gray-600 dark:text-gray-400">
            {{ t('admin.revenue.sharePolicy.selfUseCommissionRate') }}
          </label>
          <div class="relative">
            <input
              v-model="selfUseCommissionForm.ratePercent"
              type="number"
              min="0"
              max="100"
              step="0.01"
              class="input w-full pr-10"
              @blur="normalizeSelfUseCommissionPercent"
            />
            <span class="pointer-events-none absolute inset-y-0 right-3 flex items-center text-sm text-gray-400">%</span>
          </div>
          <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
            {{ t('admin.revenue.sharePolicy.selfUseCommissionHint') }}
          </p>
        </div>

        <div class="flex flex-col gap-2 md:items-start">
          <div class="text-sm text-gray-600 dark:text-gray-300">
            {{ t('admin.revenue.sharePolicy.selfUseCommissionExample', { rate: formatPercentFromNumber(Number(selfUseCommissionForm.ratePercent) || 0) }) }}
          </div>
          <button type="submit" class="btn btn-primary h-10" :disabled="savingSelfUseCommission">
            {{ savingSelfUseCommission ? t('common.saving') : t('admin.revenue.sharePolicy.saveCommission') }}
          </button>
        </div>
      </form>
    </section>
...
const savingSelfUseCommission = ref(false)
...
const selfUseCommissionForm = reactive({
...
async function loadPolicies() {
  ...
  try {
    ...
    selfUseCommissionForm.ratePercent = roundPercent((settings.user_private_group_commission_rate ?? 0) * 100)
    syncFormFromPolicy()
  } catch (err: unknown) {
...
async function saveSelfUseCommissionSettings() {
  const rateValue = Number(selfUseCommissionForm.ratePercent)
  if (!Number.isFinite(rateValue) || rateValue < 0 || rateValue > 100) {
    appStore.showError(t('admin.revenue.sharePolicy.invalidRatio'))
    return
  }

  savingSelfUseCommission.value = true
  try {
    await updateSettings({
      user_private_group_commission_rate: clampPercent(rateValue) / 100
    })
    selfUseCommissionForm.ratePercent = roundPercent(clampPercent(rateValue))
    ...
    savingSelfUseCommission.value = false
  }
}
...
function normalizeSelfUseCommissionPercent() {
  const value = Number(selfUseCommissionForm.ratePercent)
  if (!Number.isFinite(value)) {
    selfUseCommissionForm.ratePercent = 0
  } else {
    selfUseCommissionForm.ratePercent = roundPercent(clampPercent(value))
  }
}
